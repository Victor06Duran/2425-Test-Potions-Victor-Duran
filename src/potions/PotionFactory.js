  ...
  static createPotion(ingredients, diseases) {
    ...
    for (let ingredient of ingredients) {
      ...
      for (let effect of ingredient.effects) {
        ingredientAttributeType =
          attributes.find(attribute => effect.includes(attribute)) || ""
    ...
    const areAttributesTypeEqual = attributeTypes.every(
      attr => attr === attributeTypes[0]
    )
    const areEffectsTypeEqual = effectTypes.every(
      effect => effect === effectTypes[0]
    )
    const areEffectsPositive = effectPositives.every(
      pos => pos === effectPositives[0]
    )
